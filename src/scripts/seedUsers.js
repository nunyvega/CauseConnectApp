...
function clearCollections() {
  return User.deleteMany({})
    ...
    .then(() => {
      console.log(
        "All documents in the Connection collection have been deleted."
      );
    });
}
...
function generateRandomItems(path, min, max) {
  ...
  while (items.length < itemsCount) {
    const newItem = faker.random.arrayElement(
      User.schema.path(path).caster.enumValues
    );
  ...
  for (let i = 0; i < numberOfUsers; i++) {
    ...
    users.push(
      new User({
        ...
        favoriteBook: faker.random.arrayElement([
          "The Great Gatsby",
          "Moby Dick",
...
function generateConnectionsForAdmin() {
  return User.find({ username: { $in: ["admin", "admin2"] } }).then(
    (adminUsers) => {
      const adminIds = adminUsers.map((user) => user._id);
      return Promise.all(
        adminIds.map((adminId) => createConnectionsUsingAdminId(adminId))
      );
    }
  );
}
